...
function hasTopLevelColorUsage(content) {
  ...
  for (let line of lines) {
    ...
    line = line.replace(/\/\/.*$/g, '');
    // Consider top-level as depth <= 1 (inside top-level object/array literals)
    if (depth <= 2) {
      if (/Theme\.(colors|status|background|text|neutral|primary|accent|feedback)/.test(line)) {
        return true;
      }
    }
...
for (const f of files) {
  ...
  const hasImport = /import.*Theme.*from ['"]@src\/(constants\/Theme|shared\/constants\/Theme)['"]/.test(content);
  if (!hasImport) continue;
  const top = hasTopLevelColorUsage(content);
  if (top) {
    console.log('TOP ' + f);
  } else {
