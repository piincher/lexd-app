...
export const useClientPackingListScreen = (containerId: string) => {
  ...
  const generateShareText = useCallback(() => {
    ...
    return ['═══════════════════════════════════════', '       LISTE DE COLISAGE CLIENT', '═══════════════════════════════════════', '', `Container: ${containerNumber}`, `Mode: ${shippingLineLabel}`, `Route: ${route.origin} → Dakar → ${route.destination}`, `Date: ${formatDate(packingList.generatedAt)}`, ...(loadDate ? [`Date de chargement: ${formatDateTime(loadDate)}`] : []), ...(dakarPortArrivalAt ? [`Arrivée estimée au port de Dakar: ${formatDateTime(dakarPortArrivalAt)}`] : []), '', '── POINT DE RETRAIT ───────────────────', 'Entrepôt: ChinaLink Express Warehouse - Bamako', '', '── DESTINATAIRE ───────────────────────', `Nom: ${consignee.name}`, `Téléphone: ${consignee.phone}`, `Adresse: ${consignee.warehouseAddress}`, '', '── DOCUMENTS REQUIS ───────────────────', "• Pièce d'identité", '• Reçu de paiement', '', '── MARCHANDISES ───────────────────────', "N°  ID           Description          Qté  CBM    Poids", '──  ───────────  ───────────────────  ───  ─────  ──────', ...itemLines, '', '── TOTAUX ─────────────────────────────', `Total Marchandises: ${summary.totalItems} colis`, `Total Articles: ${summary.totalQuantity || totalQuantity}`, `Volume Total: ${summary.totalCBM.toFixed(2)} m³`, `Poids Total: ${summary.totalWeight.toFixed(0)} kg`, '', '── STATUT ─────────────────────────────', `Statut: ${tracking.statusLabel}`, ...(dakarPortArrivalAt ? [`Arrivée Dakar: ${formatDateTime(dakarPortArrivalAt)}`] : []), ...(signature?.signed ? ['', '── SIGNATURE ──────────────────────────', signature.signatureLabel || `Signé par ${signature.signedBy || 'ChinaLink Express'}`, `Signataire: ${signature.signerName || 'Service Logistique'}`, `Date signature: ${formatDateTime(signature.signedAt)}`] : []), '', '═══════════════════════════════════════', 'ChinaLink Express - Transport International', 'Route: Chine → Dakar (Sénégal) → Bamako (Mali)', '═══════════════════════════════════════'].join('\n');
