...
export const useGetMyPackingList = (containerId: string) => {
  return useQuery({
    ...
    select: (response) => {
      const raw = response.data.data as unknown as RawPackingListResponse | undefined;
      if (!raw) return raw;
      ...
      const dakarPortArrivalAt =
        schedule.dakarPortArrivalAt || timeline.dakarPortArrivalAt || tracking.dakarPortArrivalAt || tracking.estimatedArrival;

      const normalized: ClientPackingListResponse = {
        containerNumber: valueAsString(c?.number || c?.virtualContainerNumber || raw.containerNumber),
        ...
        tracking: {
          ...
          statusLabel: valueAsString(c?.statusLabel || tracking.statusLabel || c?.status || tracking.status),
          ...
          estimatedArrival: dakarPortArrivalAt || timeline.estimatedArrival || tracking.estimatedArrival,
          dakarPortArrivalAt,
        },
        schedule: {
          loadDate: schedule.loadDate || timeline.loadingCompletedAt || tracking.loadingCompletedAt || null,
          loadedOnVesselAt: schedule.loadedOnVesselAt || timeline.loadedOnVesselAt || tracking.loadedOnVesselAt || null,
