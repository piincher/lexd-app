...
import { amber } from "@src/shared/constants/brand";
...
export const BadgeItem: React.FC<BadgeItemProps> = ({ badge, isDark, colors }) => {
  ...
  const color = badge.earned ? tierColors[badge.tier] || tierColors.GOLD : colors.text.disabled;
    ...
    <View style={styles.badgeWrapper}>
      ...
      {!badge.earned && badge.progressPercentage > 0 && (
        <View style={[styles.miniProgressTrack, { backgroundColor: colors.background.overlay }]}>
          <View style={[styles.miniProgressFill, { width: `${Math.min(badge.progressPercentage, 100)}%`, backgroundColor: colors.accent.gold || amber[500] }]} />
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  badgeLabel: { fontFamily: Fonts.meduim, fontSize: 9, fontWeight: "700", letterSpacing: 0.6, textTransform: "uppercase", marginTop: 4, textAlign: "center" },
