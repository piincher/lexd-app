  ...
  return (
    ...
    <ScrollView ref={scrollViewRef} style={styles.scrollView} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
      <View style={styles.fieldContainer}>
        ...
        <TextInput style={styles.input} value={form.code} onChangeText={(v) => onChange("code", v.toUpperCase())} placeholder="ex: WELCOME20" placeholderTextColor={colors.text.disabled} autoCapitalize="characters" onFocus={scrollToEnd} />
      </View>

      <View style={styles.fieldContainer}>
        <Text style={styles.fieldLabel}>Nom</Text>
        <TextInput style={styles.input} value={form.name} onChangeText={(v) => onChange("name", v)} placeholder="Nom de la promotion" placeholderTextColor={colors.text.disabled} onFocus={scrollToEnd} />
      </View>

      <View style={styles.fieldContainer}>
        <Text style={styles.fieldLabel}>Description</Text>
        <TextInput style={[styles.input, styles.textArea]} value={form.description} onChangeText={(v) => onChange("description", v)} placeholder="Description (optionnel)" placeholderTextColor={colors.text.disabled} multiline numberOfLines={3} onFocus={scrollToEnd} />
      </View>
      ...
      <View style={styles.fieldContainer}>
        ...
        <TextInput style={styles.input} value={form.value} onChangeText={(v) => onChange("value", v)} placeholder={form.type === "PERCENTAGE" ? "ex: 20" : "ex: 5000"} placeholderTextColor={colors.text.disabled} keyboardType="numeric" onFocus={scrollToEnd} />
      </View>

      {form.type === "PERCENTAGE" && (
        <View style={styles.fieldContainer}>
          <Text style={styles.fieldLabel}>Réduction max (FCFA, optionnel)</Text>
          <TextInput style={styles.input} value={form.maxDiscount} onChangeText={(v) => onChange("maxDiscount", v)} placeholder="ex: 10000" placeholderTextColor={colors.text.disabled} keyboardType="numeric" onFocus={scrollToEnd} />
        </View>
      )}

      <View style={styles.fieldContainer}>
        <Text style={styles.fieldLabel}>Montant minimum commande (optionnel)</Text>
        <TextInput style={styles.input} value={form.minOrderAmount} onChangeText={(v) => onChange("minOrderAmount", v)} placeholder="ex: 20000" placeholderTextColor={colors.text.disabled} keyboardType="numeric" onFocus={scrollToEnd} />
      </View>
      ...
      <View style={styles.row}>
        <View style={[styles.fieldContainer, { flex: 1 }]}>
          ...
          <TextInput style={styles.input} value={form.maxUsages} onChangeText={(v) => onChange("maxUsages", v)} placeholder="ex: 100" placeholderTextColor={colors.text.disabled} keyboardType="numeric" onFocus={scrollToEnd} />
        </View>
        <View style={{ width: 12 }} />
        <View style={[styles.fieldContainer, { flex: 1 }]}>
          <Text style={styles.fieldLabel}>Max par utilisateur</Text>
          <TextInput style={styles.input} value={form.maxPerUser} onChangeText={(v) => onChange("maxPerUser", v)} placeholder="1" placeholderTextColor={colors.text.disabled} keyboardType="numeric" onFocus={scrollToEnd} />
        </View>
      </View>
