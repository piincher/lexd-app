...
import { styles } from "./PromoForm.styles";
    ...
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={styles.fieldContainer}>
          ...
          <TextInput style={styles.input} value={form.code} onChangeText={(v) => onChange("code", v.toUpperCase())} placeholder="ex: WELCOME20" placeholderTextColor={Theme.colors.text.muted} autoCapitalize="characters" />
        </View>

        <View style={styles.fieldContainer}>
          <Text style={styles.fieldLabel}>Nom</Text>
          <TextInput style={styles.input} value={form.name} onChangeText={(v) => onChange("name", v)} placeholder="Nom de la promotion" placeholderTextColor={Theme.colors.text.muted} />
        </View>

        <View style={styles.fieldContainer}>
          <Text style={styles.fieldLabel}>Description</Text>
          <TextInput style={[styles.input, styles.textArea]} value={form.description} onChangeText={(v) => onChange("description", v)} placeholder="Description (optionnel)" placeholderTextColor={Theme.colors.text.muted} multiline numberOfLines={3} />
        </View>
        ...
        <View style={styles.fieldContainer}>
          ...
          <TextInput style={styles.input} value={form.value} onChangeText={(v) => onChange("value", v)} placeholder={form.type === "PERCENTAGE" ? "ex: 20" : "ex: 5000"} placeholderTextColor={Theme.colors.text.muted} keyboardType="numeric" />
        </View>

        {form.type === "PERCENTAGE" && (
          <View style={styles.fieldContainer}>
            <Text style={styles.fieldLabel}>Réduction max (FCFA, optionnel)</Text>
            <TextInput style={styles.input} value={form.maxDiscount} onChangeText={(v) => onChange("maxDiscount", v)} placeholder="ex: 10000" placeholderTextColor={Theme.colors.text.muted} keyboardType="numeric" />
          </View>
        )}

        <View style={styles.fieldContainer}>
          <Text style={styles.fieldLabel}>Montant minimum commande (optionnel)</Text>
          <TextInput style={styles.input} value={form.minOrderAmount} onChangeText={(v) => onChange("minOrderAmount", v)} placeholder="ex: 20000" placeholderTextColor={Theme.colors.text.muted} keyboardType="numeric" />
        </View>
        ...
        <View style={styles.row}>
          <View style={[styles.fieldContainer, { flex: 1 }]}>
            ...
            <TextInput style={styles.input} value={form.maxUsages} onChangeText={(v) => onChange("maxUsages", v)} placeholder="ex: 100" placeholderTextColor={Theme.colors.text.muted} keyboardType="numeric" />
          </View>
          <View style={{ width: 12 }} />
          <View style={[styles.fieldContainer, { flex: 1 }]}>
            <Text style={styles.fieldLabel}>Max par utilisateur</Text>
            <TextInput style={styles.input} value={form.maxPerUser} onChangeText={(v) => onChange("maxPerUser", v)} placeholder="1" placeholderTextColor={Theme.colors.text.muted} keyboardType="numeric" />
          </View>
        </View>
        ...
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity style={[styles.submitButton, isSubmitting && styles.submitButtonDisabled]} onPress={onSubmit} disabled={isSubmitting} activeOpacity={0.7}>
            {isSubmitting ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={styles.submitButtonText}>{isEditing ? "Mettre à jour" : "Créer"}</Text>}
          </TouchableOpacity>
        </View>
      </ScrollView>
