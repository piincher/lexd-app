...
export const useContainerStatusMutations = (
  ...
  const handleUpdateStatus = useCallback(async (newStatus: ContainerStatus) => {
    ...
    const doUpdate = async () => { try { await updateStatus.mutateAsync({ id: containerId, data: { status: newStatus } }); Alert.alert('Succès', `Statut mis à jour: ${CONTAINER_STATUS_LABELS[newStatus]}`); } catch (error: unknown) { Alert.alert('Erreur', getErrorMessage(error, 'Impossible de mettre à jour le statut')); } };
